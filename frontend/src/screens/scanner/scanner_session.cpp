...
void start_snapshot_worker(AppState &state, bool refine, RefineMode mode,
  ...
  const uint32_t value_len = state.scan.snapshot_value_len != 0U
      ? state.scan.snapshot_value_len
      : current_scan_value_len(state);
  const int value_type = state.scan.snapshot_value_len != 0U
      ? state.scan.snapshot_type
      : state.scan.type;
  const bool has_batch = has_batch_read(state);
  const bool is_unknown = state.scan.is_unknown_session;
  const ScanResult original_result = state.scan.result;
  const std::vector<ScanSnapshotEntry> old_snapshot = state.scan.snapshot;
  ...
  std::string &error_out = state.scan.async_error;
...
void capture_scan_snapshot(AppState &state) {
  ...
  if (state.selected_pid <= 0 || state.scan.result.addresses.empty()) {
...
void refine_scan(AppState &state, RefineMode mode) {
  ...
  if (state.scan.snapshot.empty() || state.scan.snapshot_value_len == 0U) {
  ...
  if (mode == RefineMode::ExactValue) {
    ...
    if (!build_scan_value(state.scan.snapshot_type, state.scan.value,
                          target, target_len) ||
        target_len != state.scan.snapshot_value_len) {
