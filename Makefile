...
.PHONY: all clean host payload-ps4 payload-ps4-lib payload-ps5 payload-ps5-lib deploy-ps4 deploy-ps5 frontend gdb-bridge test-gdb-bridge test-debugger-disassembly x64dbg-plugin verify test test-aob-boundary test-process-aob-e2e test-debugger test-memory test-pal-ebadf test-pal-memory-console test-pal-memory-console-ps4 test-pal-wait-backend-ps4 test-privilege-ps4 test-ps4-debug-gate test-process-map-metadata test-process-map-cache test-lz4 test-scan-partition test-scan-protocol test-tracer-daemon test-new-features test-sjson test-legacy-scanner-e2e test-legacy-process-e2e test-reconnect-state-machine test-protocol-abi test-daemon-config-file check-locales check-headers tracer-tool fuzz-protocol-header fuzz-lz4 fuzz-sjson fuzz-process-maps fuzz-corpus FORCE
...
test-pal-memory-console: tests/test_pal_memory_console.c tests/include/ps5/kernel.h tests/include/ps5/mdbg.h $(GENERATED_VERSION_HEADER)
	...
	@echo "--- Running PAL memory console fallback test (PS5) ---"
	$(BUILD_DIR)/test_pal_memory_console

test-pal-memory-console-ps4: tests/test_pal_memory_console_ps4.c tests/include/ps4/mdbg.h $(GENERATED_VERSION_HEADER)
	@mkdir -p $(BUILD_DIR)
...
test-ps4-debug-gate: tests/test_ps4_debug_gate.c tests/include/ps4/kernel.h $(GENERATED_VERSION_HEADER)
	@mkdir -p $(BUILD_DIR)
	$(HOST_CC) $(HOST_CPPFLAGS) -Itests/include $(HOST_CFLAGS) -DPLATFORM_PS4=1 tests/test_ps4_debug_gate.c $(HOST_LDFLAGS) $(HOST_LDLIBS) -o $(BUILD_DIR)/test_ps4_debug_gate
	@echo "--- Running PS4 debug gate arming test ---"
	$(BUILD_DIR)/test_ps4_debug_gate
...
test-pal-wait-backend-ps4: tests/test_pal_wait_backend_ps4.c src/pal/pal_wait.c
	@mkdir -p $(BUILD_DIR)
	$(HOST_CC) $(HOST_CPPFLAGS) $(HOST_CFLAGS) -DPLATFORM_PS4=1 $^ $(HOST_LDFLAGS) -o $(BUILD_DIR)/test_pal_wait_backend_ps4
	@echo "--- Running PS4 wait backend selection test ---"
	$(BUILD_DIR)/test_pal_wait_backend_ps4
...
test-daemon-config-file: tests/test_daemon_config_file.c src/core/daemon/daemon_config_file.c
	@mkdir -p $(BUILD_DIR)
	$(HOST_CC) $(HOST_CPPFLAGS) $(HOST_CFLAGS) tests/test_daemon_config_file.c \
		src/core/daemon/daemon_config_file.c $(HOST_LDFLAGS) $(HOST_LDLIBS) \
		-o $(BUILD_DIR)/test_daemon_config_file
	@echo "--- Running daemon.conf unit tests ---"
	$(BUILD_DIR)/test_daemon_config_file
...
test: test-aob-boundary test-process-aob-e2e test-debugger test-memory test-pal-ebadf test-pal-memory-console test-pal-memory-console-ps4 test-pal-wait-backend-ps4 test-privilege-ps4 test-ps4-debug-gate test-process-map-metadata test-process-map-cache test-debugger-e2e test-debugger-protocol test-lz4 test-scan-partition test-scan-protocol test-tracer-daemon test-new-features test-sjson test-protocol-abi test-daemon-config-file test-legacy-scanner-e2e test-legacy-process-e2e test-thread-pool test-max-connections-e2e test-idle-timeout-e2e test-idle-timeout-unit test-kqueue-timeout test-reconnect-e2e test-reconnect-state-machine test-reconnect-50-restarts test-memdbg-instance fuzz-corpus
...
gdb-bridge:
	...
	cmake --build $(BUILD_DIR)/gdb_bridge --config Release -j4 --target memdbg_gdb_bridge memdbg_gdb_bridge_test

test-gdb-bridge: gdb-bridge
	@if [ -x "$(BUILD_DIR)/gdb_bridge/Release/memdbg_gdb_bridge_test.exe" ]; then \
	  "$(BUILD_DIR)/gdb_bridge/Release/memdbg_gdb_bridge_test.exe"; \
	elif [ -x "$(BUILD_DIR)/gdb_bridge/Release/memdbg_gdb_bridge_test" ]; then \
